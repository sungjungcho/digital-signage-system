...
export default function PatientManager() {
  ...
  const fetchPatients = async () => {
    ...
    try {
      const response = await fetch(`/api/devices/${selectedDeviceId}/patients`);
      if (response.ok) {
        const data = await response.json();
        setPatients(data);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingPatient
        ? `/api/devices/${selectedDeviceId}/patients/${editingPatient.id}`
        : `/api/devices/${selectedDeviceId}/patients`;
  ...
  const handleDelete = async (patient: Patient) => {
    ...
    try {
      const response = await fetch(`/api/devices/${selectedDeviceId}/patients/${patient.id}`, {
        method: 'DELETE',
      });
  ...
  const handlePatientCall = async (patient: Patient) => {
    try {
      ...
      // 알림 전송 성공 후 환자 삭제
      const deleteResponse = await fetch(`/api/devices/${selectedDeviceId}/patients/${patient.id}`, {
        method: 'DELETE',
      });
      ...
      <div>
        ...
        <div className="mb-4">
          ...
          <select
            ...
            className="w-full md:w-1/2 h-[42px] px-3 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          >
